...
public interface GraphWorkflowNode<S> {
     ...
     */
    WorkflowResult<GraphCommand<S>, WorkflowError> process(ModernGraphWorkflowState<S> state);
    ...
    default GraphNodeMetadata getMetadata() {
        return GraphNodeMetadata.of(getNodeId(), getName(), getDescription(), GraphNodeMetadata.NodeType.TASK);
    }
     ...
     */
    default WorkflowResult<ModernGraphWorkflowState<S>, WorkflowError> validateState(ModernGraphWorkflowState<S> state) {
     ...
     */
    default void onBeforeExecution(ModernGraphWorkflowState<S> state) {
        // Default: no action
    }
     ...
     */
    default void onAfterExecution(ModernGraphWorkflowState<S> state, GraphCommand<S> command) {
        // Default: no action
    }
     ...
     */
    default void onExecutionError(ModernGraphWorkflowState<S> state, WorkflowError error) {
        // Default: no action
    }
     ...
     */
    default WorkflowResult<GraphCommand<S>, WorkflowError> processWithLifecycle(ModernGraphWorkflowState<S> state) {
        try {
            ...
            return WorkflowResult.suspended("execution-error", state.data(), 
...
record GraphNodeMetadata(
    ...
    public GraphNodeMetadata {
        ...
        Objects.requireNonNull(expectedDuration, "Expected duration optional cannot be null");
    ...
    public static GraphNodeMetadata of(NodeId nodeId, String name, String description, NodeType type) {
        return new GraphNodeMetadata(nodeId, name, description, type, 
            Collections.emptySet(), Collections.emptyMap(), Optional.empty());
    }
    
    public static GraphNodeMetadata withTags(NodeId nodeId, String name, String description, 
                                           NodeType type, Set<String> tags) {
        return new GraphNodeMetadata(nodeId, name, description, type, tags, 
            Collections.emptyMap(), Optional.empty());
    }
    
    public static GraphNodeMetadata withDuration(NodeId nodeId, String name, String description, 
                                                NodeType type, Duration expectedDuration) {
        return new GraphNodeMetadata(nodeId, name, description, type, 
            Collections.emptySet(), Collections.emptyMap(), Optional.of(expectedDuration));
    }
    
    public GraphNodeMetadata withTag(String tag) {
        var newTags = new java.util.HashSet<>(tags);
        newTags.add(tag);
        return new GraphNodeMetadata(nodeId, name, description, type, newTags, properties, expectedDuration);
    }
    
    public GraphNodeMetadata withProperty(String key, Object value) {
        var newProperties = new java.util.HashMap<>(properties);
        newProperties.put(key, value);
        return new GraphNodeMetadata(nodeId, name, description, type, tags, newProperties, expectedDuration);
    }
