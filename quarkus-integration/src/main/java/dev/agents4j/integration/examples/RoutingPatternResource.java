...
public class RoutingPatternResource {
        ...
        try {
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(
                    Map.of(
                        "error",
                        "Failed to execute customer support routing: " +
                        e.getMessage()
                    )
                )
                .build();
    ...
    public Response contentCategorization(
        ContentCategorizationRequest request
    ) {
        if (!workflowsEnabled) {
            return Response.status(Response.Status.SERVICE_UNAVAILABLE)
                .entity(Map.of("error", "Workflows are disabled"))
        ...
        try {
            ...
            String result =
                ...
                request
                    .getContent()
                    .substring(0, Math.min(50, request.getContent().length())) +
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(
                    Map.of(
                        "error",
                        "Failed to execute content categorization: " +
                        e.getMessage()
                    )
                )
                .build();
        ...
        try {
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(
                    Map.of(
                        "error",
                        "Failed to execute multi-language routing: " +
                        e.getMessage()
                    )
                )
                .build();
    ...
    public Response llmRouting(LLMRoutingRequest request) {
        ...
        try {
            ...
            String classificationResult =
                "Classification: technical_support (confidence: 0.85)";
            if (
                request.getIncludeReasoning() != null &&
                request.getIncludeReasoning()
            ) {
                classificationResult +=
                    " - Reasoning: Contains technical terminology and API references";
            }
            String result =
            ...
            response.put(
                ...
                request.getClassificationPrompt()
            );
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(
                    Map.of(
                        "error",
                        "Failed to execute LLM routing: " + e.getMessage()
                    )
                )
                .build();
        }
    ...
    public Response ruleBasedRouting(RuleBasedRoutingRequest request) {
        ...
        try {
            ...
            String matchedRoute = request.getDefaultRouteId() != null
                ? request.getDefaultRouteId()
                : "general_inquiry";
            ...
            for (RoutingRule rule : request.getRules()) {
                if (
                    request
                        .getInput()
                        .toLowerCase()
                        .matches(rule.getPattern().toLowerCase())
                ) {
            ...
            String result =
                "Rule-based routing result: Input routed to '" +
                matchedRoute +
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(
                    Map.of(
                        "error",
                        "Failed to execute rule-based routing: " +
                        e.getMessage()
                    )
                )
                .build();
        ...
        try {
            ...
            switch (exampleType.toLowerCase()) {
                case "customer-support":
                    result =
                        "Customer support routing example: Intelligent routing of support tickets to specialized agents";
                    break;
                case "content-categorization":
                    result =
                        "Content categorization example: Automatic categorization of content by topic and type";
                    break;
                case "multi-language":
                    result =
                        "Multi-language routing example: Language detection and routing to appropriate processors";
                    break;
                case "intelligent-routing":
                    result =
                        "Intelligent routing example: AI-powered content analysis and smart routing decisions";
                    break;
                case "priority-routing":
                    result =
                        "Priority routing example: Urgent content identification and priority-based routing";
                    break;
                default:
    ...
    public Response getRoutingTypes() {
        ...
        types.put(
            "customer_support",
            "Route customer queries to appropriate support specialists"
        );
        types.put(
            "content_categorization",
            "Categorize and route content based on topic/type"
        );
        types.put("multi_language", "Route content based on detected language");
        types.put(
            "llm_based",
            "Use LLM for intelligent content analysis and routing"
        );
        ...
        examples.put(
            ...
            Map.of(
                "customer_support",
                "Help desk, technical support, sales inquiries",
                "content_categorization",
                "Document management, content moderation",
                "multi_language",
                "International applications, translation services",
                "llm_based",
                "Complex decision making, context-aware routing",
                "rule_based",
                "Simple pattern matching, keyword-based routing"
            )
        );
    ...
    public Response analyzeRouting(RoutingAnalysisRequest request) {
        ...
        try {
            ...
            analysis.put(
                ...
                input
                    .toLowerCase()
                    .matches(".*\\b(help|support|problem|issue|error)\\b.*")
            );
            ...
            if (
                input.toLowerCase().contains("support") ||
                input.toLowerCase().contains("help")
            ) {
                recommendations.put("primary", "customer_support");
                recommendations.put(
                    "reason",
                    "Contains support-related keywords"
                );
            } else if (input.length() > 500) {
                recommendations.put("primary", "content_categorization");
                recommendations.put(
                    "reason",
                    "Long content suitable for categorization"
                );
            } else {
                recommendations.put("primary", "llm_based");
                recommendations.put(
                    "reason",
                    "Best for general-purpose intelligent routing"
                );
            }
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(
                    Map.of(
                        "error",
                        "Failed to analyze routing: " + e.getMessage()
                    )
                )
                .build();
        }
    ...
    private String detectLanguage(String text) {
        ...
        if (
            ...
            text
                .toLowerCase()
                .matches(
                    ".*\\b(el|la|de|que|y|a|en|un|es|se|no|te|lo|le|da|su|por|son|con|para|una|del|al|está|como|mu|bien|ha|me|si|sin|sobre|este|ya|entre|cuando|todo|esta|ser|son|dos|también|era|muy|años|hasta|desde|está|mi|porque|qué|solo|han|yo|hay|vez|puede|todos|así|nos|ni|parte|tiene|él|uno|donde|mucho|sea|ella|sus|había|hecho|cada|fue|puede|nombre|mismo|dio|tres|menos|debe|casa|poco|año|antes|mundo|aquí|menos|dijo|otra)\\b.*"
                )
        ) {
            return "spanish";
        } else {
    ...
    public static class LLMRoutingRequest {
        ...
        public void setClassificationPrompt(String classificationPrompt) {
