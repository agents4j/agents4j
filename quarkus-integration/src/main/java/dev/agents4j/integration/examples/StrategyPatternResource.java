...
public class StrategyPatternResource {
        ...
        try {
                    ...
                    .append(
                        request
                            ...
                            .substring(
                                ...
                                Math.min(
                                    30,
                                    request.getSystemPrompts().get(i).length()
                                )
                            )
                    )
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(
                    Map.of(
                        "error",
                        "Failed to execute sequential strategy: " +
                        e.getMessage()
                    )
                )
                .build();
        ...
        try {
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(
                    Map.of(
                        "error",
                        "Failed to execute parallel strategy: " + e.getMessage()
                    )
                )
                .build();
        }
    ...
    public Response conditionalExecution(ConditionalStrategyRequest request) {
        ...
        try {
            ...
            boolean shortCircuit = request.getShortCircuit() != null
                ? request.getShortCircuit()
                : false;
            boolean requireAtLeastOne = request.getRequireAtLeastOne() != null
                ? request.getRequireAtLeastOne()
                : true;
            ...
            result
                ...
                .append(requireAtLeastOne)
                .append("\n");

            int executedAgents = shortCircuit
                ? 1
                : request.getSystemPrompts().size();
            for (int i = 0; i < executedAgents; i++) {
                result
                    .append("Agent ")
                    .append(i + 1)
                    .append(": Condition met, executed\n");
            }
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(
                    Map.of(
                        "error",
                        "Failed to execute conditional strategy: " +
                        e.getMessage()
                    )
                )
                .build();
    ...
    public Response batchExecution(BatchStrategyRequest request) {
        ...
        try {
            ...
            int batchSize = request.getBatchSize() != null
                ...
                : 5;
            boolean parallelBatches = request.getParallelBatches() != null
                ? request.getParallelBatches()
                : true;
            ...
            result
                .append("Parallel batches: ")
                .append(parallelBatches)
                .append("\n");
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(
                    Map.of(
                        "error",
                        "Failed to execute batch strategy: " + e.getMessage()
                    )
                )
                .build();
        }
    ...
    public Response getExample(@PathParam("exampleType") String exampleType) {
        ...
        try {
            ...
            switch (exampleType.toLowerCase()) {
                case "sequential":
                    result =
                        "Sequential strategy example: Agents process input one after another in order";
                    break;
                case "parallel":
                    result =
                        "Parallel strategy example: All agents process input simultaneously";
                    break;
                case "conditional":
                    result =
                        "Conditional strategy example: Agents execute based on conditions and criteria";
                    break;
                case "batch":
                    result =
                        "Batch strategy example: Large datasets processed in configurable batch sizes";
                    break;
                case "adaptive":
                    result =
                        "Adaptive strategy example: Strategy selection based on input characteristics";
                    break;
                default:
    ...
    public Response getStrategyTypes() {
        ...
        examples.put(
            ...
            Map.of(
                "sequential",
                "When output from one agent feeds into the next",
                "parallel",
                "When agents work independently on the same input",
                "conditional",
                "When execution depends on input characteristics",
                "batch",
    ...
    public Response compareStrategies(CompareStrategiesRequest request) {
        ...
        try {
            ...
            String sequentialResult =
                "Sequential result: All " +
                request.getSystemPrompts().size() +
                " agents processed sequentially";
            ...
            results.put(
                ...
                Map.of(
                    ...
                    sequentialTime > 0
                        ...
                        : 0,
                    "recommendation",
                    parallelTime < sequentialTime ? "parallel" : "sequential"
                )
            );
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(
                    Map.of(
                        "error",
                        "Failed to compare strategies: " + e.getMessage()
                    )
                )
                .build();
        }
    ...
    public Response getHealthInfo() {
        ...
        health.put(
            ...
            List.of("sequential", "parallel", "conditional", "batch")
        );
    ...
    public static class ConditionalStrategyRequest extends StrategyRequest {
        ...
        public Boolean getRequireAtLeastOne() {
    ...
    public static class BatchStrategyRequest extends StrategyRequest {
        ...
        public Boolean getParallelBatches() {
            return parallelBatches;
        }

        public void setParallelBatches(Boolean parallelBatches) {
