...
class SimpleGraphWorkflowImplTest {
    ...
    private static final ContextKey<Integer> STEP_COUNT = ContextKey.intKey(
        "step.count"
    );
    ...
    void shouldExecuteSimpleLinearWorkflow() {
        ...
        assertEquals(
            "Output from workflow: test-input",
            result.getValue().get()
        );
    ...
    void shouldMaintainContextThroughoutWorkflow() {
        ...
        startNode.setContextUpdater(ctx ->
            ctx.with(STEP_COUNT, ctx.get(STEP_COUNT).orElse(0) + 1)
        );
        middleNode.setContextUpdater(ctx ->
            ctx.with(STEP_COUNT, ctx.get(STEP_COUNT).orElse(0) + 1)
        );
        endNode.setContextUpdater(ctx ->
            ctx.with(STEP_COUNT, ctx.get(STEP_COUNT).orElse(0) + 1)
        );
        ...
        GraphWorkflow<String, String> countingWorkflow =
            GraphWorkflowBuilder.<String, String>create(String.class)
        ...
        // Act
        WorkflowResult<String, WorkflowError> result = countingWorkflow.start(
            input,
            initialContext
        );
    ...
    void shouldSuspendAndResumeWorkflow() {
        ...
        WorkflowResult<String, WorkflowError> startResult = workflow.start(
            input
        );
        ...
        assertEquals(
            "test-suspension",
            startResult.getSuspension().get().suspensionId()
        );

        // Get the suspended state using type-safe extraction
        Optional<WorkflowSuspension<GraphWorkflowState<String>>> typedSuspension = workflow.extractTypedSuspension(startResult);
        assertTrue(typedSuspension.isPresent());
        GraphWorkflowState<String> suspendedState = typedSuspension
            .get()
            .getSuspendedState();
    ...
    void shouldProvideAccessToWorkflowStructure() {
        ...
        Set<GraphEdge> startEdges = workflow.getEdgesFrom(
            startNode.getNodeId()
        );
        ...
        assertTrue(
            foundEdgeToMiddle,
            "Should find edge from start to middle node"
        );
    }
    ...
    private static class CountingNode implements GraphWorkflowNode<String> {
        ...
        public WorkflowResult<GraphCommand<String>, WorkflowError> process(
            ...
            WorkflowContext updatedContext = contextUpdater.apply(
                state.context()
            );
