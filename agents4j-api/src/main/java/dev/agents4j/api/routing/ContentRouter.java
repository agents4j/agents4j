...
public interface ContentRouter<T> extends GraphWorkflowNode<T> {
     ...
     */
    RoutingDecision analyzeContent(T content, Set<NodeId> availableRoutes, WorkflowContext context);
    ...
    default CompletableFuture<RoutingDecision> analyzeContentAsync(
            T content, Set<NodeId> availableRoutes, WorkflowContext context) {
     ...
     */
    default EdgeCondition createRoutingCondition(NodeId targetNode, RoutingDecision routingDecision) {
    ...
    default GraphCommand<T> handleRoutingFailure(
            T content, Set<NodeId> availableRoutes, WorkflowContext context, String reason) {
        
        var fallbackNode = getRoutingStrategy().getFallbackNode();
        if (fallbackNode.isPresent()) {
            return GraphCommand.Traverse.toWithReason(fallbackNode.get(), 
                "Routing failed: " + reason + ". Using fallback route.");
        }
        
        return GraphCommand.Suspend.withId("routing-failure", 
     ...
     */
    default boolean canRoute(T content, Set<NodeId> availableRoutes, WorkflowContext context) {
        return content != null && 
    ...
    default RoutingPerformance getRoutingPerformance(
        ...
        return RoutingPerformance.builder()
            ...
            .contentType(content != null ? content.getClass().getSimpleName() : "null")
            .routeCount(availableRoutes != null ? availableRoutes.size() : 0)
            .estimatedProcessingTimeMs(estimateProcessingTime(content, availableRoutes))
            .build();
    }
     ...
     */
    default long estimateProcessingTime(T content, Set<NodeId> availableRoutes) {
        // Default implementation provides a simple estimate
        int baseTime = 50; // Base processing time in ms
        int routeComplexity = availableRoutes != null ? availableRoutes.size() * 10 : 0;
     ...
     */
    @Override
    default WorkflowResult<GraphCommand<T>, WorkflowError> process(GraphWorkflowState<T> state) {
        try {
            ...
            if (!canRoute(content, availableRoutes, context)) {
                return WorkflowResult.success(
                    handleRoutingFailure(content, availableRoutes, context, 
                        "Router cannot handle this content or route configuration")
                );
            }
            
            // Perform content analysis
            var routingDecision = analyzeContent(content, availableRoutes, context);
            
            // Check confidence threshold
            if (!routingDecision.meetsThreshold(getMinimumConfidenceThreshold())) {
                return WorkflowResult.success(
                    handleRoutingFailure(content, availableRoutes, context,
                        String.format("Confidence %.2f below threshold %.2f", 
                            routingDecision.getConfidence(), getMinimumConfidenceThreshold()))
                );
            }
            ...
            var updatedContext = context
                .with(getRoutingStrategy().getConfidenceContextKey(), routingDecision.getConfidence())
                .with(getRoutingStrategy().getReasoningContextKey(), routingDecision.getReasoning());
            
            // Add routing metadata to context
            for (Map.Entry<String, Object> entry : routingDecision.getMetadata().entrySet()) {
            ...
            var traverseCommand = GraphCommand.Traverse.toWithUpdates(targetNode, updatedContext, content);
            return WorkflowResult.success(traverseCommand);
            
        } catch (Exception e) {
            return WorkflowResult.success(
                handleRoutingFailure(state.data(), extractAvailableRoutes(state), state.context(),
    ...
    default Set<NodeId> extractAvailableRoutes(GraphWorkflowState<T> state) {
        var routesContext = state.getContext(getRoutingStrategy().getRoutesContextKey());
        if (routesContext.isPresent() && routesContext.get() instanceof Set<?>) {
            @SuppressWarnings("unchecked")
            Set<String> routeStrings = (Set<String>) routesContext.get();
            return routeStrings.stream()
                .map(NodeId::of)
    ...
    default String getDescription() {
        return String.format("Content router: %s - Routes content based on %s", 
            getRouterName(), getRoutingStrategy().getDescription());
    }
