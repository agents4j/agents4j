...
class ModernizationDemoTest {
    ...
    void demonstrateFunctionalErrorHandling() {
        ...
        // Success case
        var successResult = WorkflowResult.<String, WorkflowError>success("Hello World");
        ...
        var suspendedResult = WorkflowResult.<String, WorkflowError>suspended("test-id", "state", "testing");
    ...
    void demonstrateWorkflowResultPatternMatching() {
        ...
        var successResult = WorkflowResult.<String, WorkflowError>success("test");
        var suspendedResult = WorkflowResult.<String, WorkflowError>suspended("id", "state", "reason");
    ...
    private String getResultDescription(WorkflowResult<String, WorkflowError> result) {
        if (result instanceof WorkflowResult.Success) {
            ...
            return "Suspended: " + result.getSuspension().map(s -> s.reason()).orElse("unknown");
        } else {
            return "Other";
        }
    }
    ...
    void demonstrateCompleteWorkflowPattern() {
        ...
        var validationResult = validateUser(initialContext.get(userIdKey).orElse(""));
    ...
    private WorkflowResult<String, WorkflowError> validateUser(String userId) {
        if (userId == null || userId.trim().isEmpty()) {
            return WorkflowResult.suspended("validation-pending", userId, "User ID validation required");
        }
        if (!userId.startsWith("user-")) {
            return WorkflowResult.suspended("format-check", userId, "User ID format validation required");
        }
        return WorkflowResult.success("Validation passed");
    }
    
    private WorkflowResult<String, WorkflowError> processUser(WorkflowContext context) {
        var userId = context.get(ContextKey.stringKey("user.id"));
        var validated = context.get(ContextKey.booleanKey("validated"));
        
        if (userId.isEmpty()) {
            return WorkflowResult.suspended("user-missing", null, "User ID required");
        }
        if (!validated.orElse(false)) {
            return WorkflowResult.suspended("not-validated", userId.get(), "User validation required");
        }
