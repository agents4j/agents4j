...
class ModernStatefulWorkflowTest {
    ...
    void setUp() {
        ...
        highValueOrderData = new OrderData(
            "ORD-999",
            "CUST-VIP",
            List.of(new OrderItem("PROD-PREMIUM", 1, 1500.0)),
            OrderStatus.PENDING,
            1500.0
        );
    ...
    void shouldStartWorkflowWithTypeSafeContext() {
        ...
        assertEquals(
            ...
            execution.getStateData().orderId()
        );
        ...
        assertEquals(
            "ORD-123",
            context.get(ContextKey.stringKey("order.id")).orElse("")
        );
        assertEquals(
            "CUST-456",
            context.get(ContextKey.stringKey("customer.id")).orElse("")
        );
        assertEquals(
            175.0,
            context.get(ContextKey.of("total.amount", Double.class)).orElse(0.0)
        );
        assertEquals(
            "api",
            context.get(ContextKey.stringKey("source")).orElse("")
        );
        assertEquals(
            "us-east-1",
            context.get(ContextKey.stringKey("region")).orElse("")
        );
    }
    ...
    void shouldSuspendHighValueOrdersForApproval() {
        ...
        if (execution.isSuspended()) {
            ...
            assertEquals(
                highValueOrderData.orderId(),
                suspendedExecution.getStateData().orderId()
            );

            // Verify context contains approval flag
            var context = suspendedExecution.getContext();
            Boolean requiresApproval = context
                .get(ContextKey.booleanKey("requires.approval"))
                .orElse(false);
    ...
    void shouldResumeSuspendedWorkflows() {
        ...
        if (execution.isSuspended()) {
            ...
            assertEquals(
                ...
                resumedExecution.getExecutionId()
            );
    ...
    void shouldSupportAsyncExecution() {
        CompletableFuture<
            WorkflowResult<
                StatefulWorkflow.WorkflowExecution<OrderData, OrderResult>,
                WorkflowError
            >
        ...
        assertFalse(
            futureResult.isDone() && futureResult.isCompletedExceptionally()
        );
    ...
    void shouldDemonstrateFunctionalComposition() {
        var result = workflow
            .start(validOrderData, initialContext)
            .flatMap(execution -> {
                if (execution.isCompleted()) {
                    ...
                    return WorkflowResult.success(
                        "Workflow suspended as expected"
                    );
                } else {
    ...
    void shouldProvideTypeSafeContextOperations() {
        ...
        String orderId = context
            .get(ContextKey.stringKey("order.id"))
            .orElse("unknown");
        Double amount = context
            .get(ContextKey.of("total.amount", Double.class))
            .orElse(0.0);
        Integer retryCount = context
            .get(ContextKey.intKey("retry.count"))
            .orElse(-1);
        ...
        var newContext = context.with(
            ContextKey.stringKey("new.field"),
            "test-value"
        );
    ...
    void shouldDemonstrateErrorRecoveryPatterns() {
        ...
        var result = workflow
            ...
            .onSuccess(execution -> {
                System.out.println(
                    "Workflow execution created: " + execution.getExecutionId()
                );
            });
    ...
    void shouldHandleConcurrentExecutionSafely() {
        ...
        var futures = List.of(
            ...
            workflow.startAsync(
                validOrderData.withStatus(OrderStatus.PENDING),
                initialContext
            )
        );
        ...
        var executionIds = results
            .stream()
            .map(WorkflowResult::getOrThrow)
            .map(StatefulWorkflow.WorkflowExecution::getExecutionId)
            .toList();
