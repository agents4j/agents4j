...
import dev.agents4j.api.exception.AgentException;
...
class ExceptionSystemTest {
    ...
    void testWorkflowExecutionExceptionWithMessage() {
        ...
        WorkflowExecutionException exception = new WorkflowExecutionException(workflowName, message);
        
        assertEquals(ErrorCode.WORKFLOW_EXECUTION_FAILED, exception.getErrorCode());
        assertEquals(workflowName, exception.getWorkflowName());
        assertTrue(exception.getMessage().contains(message));
        assertTrue(exception.getMessage().contains(ErrorCode.WORKFLOW_EXECUTION_FAILED.getCode()));
    ...
    void testWorkflowExecutionExceptionWithCause() {
        ...
        WorkflowExecutionException exception = new WorkflowExecutionException(workflowName, message, cause);
    ...
    void testWorkflowExecutionExceptionWithContext() {
        ...
        WorkflowExecutionException exception = new WorkflowExecutionException(workflowName, message, context);
    ...
    void testValidationException() {
        ...
        assertTrue(exception.getMessage().contains(message));
        assertEquals(validationErrors, exception.getContext().get("validationErrors"));
    }
    ...
    void testValidationExceptionWithContext() {
        ...
        ValidationException exception = new ValidationException(message, validationErrors, context);
    ...
    void testAgentExceptionContextModification() {
        WorkflowExecutionException exception = new WorkflowExecutionException("TestWorkflow", "Test message");
    ...
    void testErrorCodeCategories() {
        ...
        assertTrue(ErrorCode.INVALID_CONFIGURATION.getCode().startsWith("AGT-1"));
        assertTrue(ErrorCode.MISSING_REQUIRED_PARAMETER.getCode().startsWith("AGT-1"));
        
        // Test workflow error codes are in 2000 range
        assertTrue(ErrorCode.WORKFLOW_EXECUTION_FAILED.getCode().startsWith("AGT-2"));
        assertTrue(ErrorCode.WORKFLOW_TIMEOUT.getCode().startsWith("AGT-2"));
        assertTrue(ErrorCode.WORKFLOW_INTERRUPTED.getCode().startsWith("AGT-2"));
        
        // Test agent error codes are in 3000 range
        assertTrue(ErrorCode.AGENT_PROCESSING_FAILED.getCode().startsWith("AGT-3"));
        assertTrue(ErrorCode.AGENT_INITIALIZATION_FAILED.getCode().startsWith("AGT-3"));
        ...
        assertTrue(ErrorCode.PROVIDER_AUTHENTICATION_FAILED.getCode().startsWith("AGT-4"));
    ...
    void testExceptionMessageFormatting() {
        ...
        WorkflowExecutionException exception = new WorkflowExecutionException("TestWorkflow", message);
