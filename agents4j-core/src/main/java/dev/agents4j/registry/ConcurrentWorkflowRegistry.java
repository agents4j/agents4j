...
public class ConcurrentWorkflowRegistry implements WorkflowRegistry {
    ...
    public List<WorkflowProviderInfo> listProviders() {
        ...
        return providersCopy.entrySet().stream()
            .map(entry -> {
    ...
    public ValidationResult validateProviders() {
        ...
        for (Map.Entry<String, WorkflowProvider<?, ?>> entry : providersCopy.entrySet()) {
            String type = entry.getKey();
            WorkflowProvider<?, ?> provider = entry.getValue();
     ...
     */
    public void replaceAllProviders(Map<String, WorkflowProvider<?, ?>> newProviders) 
        throws WorkflowRegistrationException {
        if (newProviders == null) {
            throw new IllegalArgumentException("New providers map cannot be null");
        }

        // Validate all providers first
        for (Map.Entry<String, WorkflowProvider<?, ?>> entry : newProviders.entrySet()) {
            String type = entry.getKey();
            WorkflowProvider<?, ?> provider = entry.getValue();
            
            if (type == null || type.trim().isEmpty()) {
                throw new WorkflowRegistrationException(type, "Workflow type cannot be null or empty");
            }
            if (provider == null) {
                throw new WorkflowRegistrationException(type, "Provider cannot be null");
            }
            
            try {
                provider.validate();
            } catch (WorkflowProviderException e) {
                throw new WorkflowRegistrationException(type, 
                    "Provider validation failed: " + e.getMessage(), e);
            }
        }
